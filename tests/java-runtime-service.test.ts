...
const makeManagedRuntime = async (
  ...
  const executable = path.join(binDirectory, executableName);
...
test('64-bit環境では32-bit Javaを自動選択しない', async (t) => {
  ...
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  await makeManagedRuntime(root, 'liberica-lite-8-a-x86', 'java.exe');
  await makeManagedRuntime(root, 'liberica-lite-8-z-x64', 'java.exe');
...
test('Temurin取得失敗時はローカルのMojang公式Javaへフォールバックする', async (t) => {
  ...
  await fs.mkdir(binDirectory, { recursive: true });
  const executable = path.join(binDirectory, 'java.exe');
  await fs.writeFile(executable, 'fake java');
