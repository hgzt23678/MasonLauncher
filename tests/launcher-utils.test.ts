...
test('Fabric継承versionからloader情報を復元する', () => {
  const normalized = normalizeLaunchProfileVersion(
    {
      versionId: 'stale',
      minecraftVersion: 'stale',
    ...
    {
      id: '1.21.1-fabric0.16.14',
      inheritsFrom: '1.21.1',
    },
  );
  assert.equal(normalized.loader, 'fabric');
  assert.equal(normalized.minecraftVersion, '1.21.1');
  assert.equal(normalized.loaderVersion, '0.16.14');
  assert.equal(normalized.resolvedVersionId, '1.21.1-fabric0.16.14');
});

test('NeoForge profileのversion IDを生成する', () => {
  const normalized = normalizeLaunchProfileVersion({
    versionId: '1.21.1',
    minecraftVersion: '1.21.1',
    resolvedVersionId: 'stale',
  ...
  assert.equal(normalized.resolvedVersionId, 'neoforge-21.1.200');
  assert.equal(normalized.loader, 'neoforge');
});
