...
const messages = {
  ...
  'settings.developerSettingsDescription': { ja: 'Debugビルド専用の認証と表示テーマを設定します。', en: 'Configure authentication and appearance options available only in debug builds.', zhHant: '設定僅供 Debug 組建使用的驗證與外觀選項。', zhHans: '配置仅供 Debug 构建使用的身份验证和外观选项。', ko: 'Debug 빌드 전용 인증 및 표시 테마를 설정합니다.' },
